...
import { ResolveHazmat } from '../../../src/tasks/api/ResolveHazmat';
import { ResolveNmfc } from '../../../src/tasks/api/ResolveNmfc';
import { ResolveAccessorials } from '../../../src/tasks/api/ResolveAccessorials';
...
When('the actor resolves NMFC codes for all commodities', async () => {
  await actorInTheSpotlight().attemptsTo(ResolveNmfc.forAllCommodities());
});

When('the actor resolves hazmat information for hazardous commodities', async () => {
  await actorInTheSpotlight().attemptsTo(ResolveHazmat.forHazardousCommodities());
});

When('the actor resolves accessorials for the quote', async () => {
  await actorInTheSpotlight().attemptsTo(ResolveAccessorials.forQuote());
});
