...
export class ConfirmBookingModal extends Interaction {
  ...
  async performAs(actor: UsesAbilities & AnswersQuestions): Promise<void> {
    ...
    while (attempts < 3) {
      await performing.attemptsTo(Click.on(BookingPage.confirmBookingButton.nth(0)), Wait.for(Duration.ofSeconds(1)));

      try {
        await performing.attemptsTo(Wait.until(BookingPage.confirmBookingButton.nth(1), isVisible()));
        return;
      } catch {
        attempts++;
      }
