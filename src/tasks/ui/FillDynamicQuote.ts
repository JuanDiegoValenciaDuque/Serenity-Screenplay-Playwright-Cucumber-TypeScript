  ...
  async performAs(actor: UsesAbilities & PerformsActivities): Promise<void> {
    ...
    for (let i = 0; i < commodities.length; i++) {
      ...
      if (i > 0) {
        ...
        await actor.attemptsTo(
          Press.the('Tab'),
          Wait.upTo(Duration.ofSeconds(10)).until(QuotingPage.ItemNameInputs.count(), equals(expectedCount)),
        );
      }
    ...
    await actor.attemptsTo(
      ...
      Wait.upTo(Duration.ofSeconds(120)).until(QuotingPage.PriceRates.first(), isVisible()),
    );
  }
}
